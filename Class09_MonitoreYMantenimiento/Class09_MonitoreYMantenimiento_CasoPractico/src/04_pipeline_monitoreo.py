"""
src/04_pipeline_monitoreo.py — Pipeline automatizado de monitoreo con alertas.
...
Uso:
    python src/04_pipeline_monitoreo.py              # lote actual
    python src/04_pipeline_monitoreo.py wine_2024_Q2 # con nombre de lote
...
# ── Umbrales de alerta ────────────────────────────────────────────────────────
DRIFT_SHARE_UMBRAL   = 1.01 #0.30   # > 30% features con drift → ALERTA
...
def ejecutar_monitoreo(nombre_lote: str = "lote_actual") -> dict:
    ...
    for p in [ref_path, prod_path]:
        if not p.exists():
            raise FileNotFoundError(
                f"{p} no encontrado. Ejecuta los pasos 1 y 2 primero."
            )
    ...
    # Extraer métricas de drift del resultado
...
if __name__ == "__main__":
    ...
    print("\n" + "="*55)
    print(f" PIPELINE DE MONITOREO — {nombre_lote}")
    print("="*55)
    ...
    # Quality gate: exit(1) si el estado es CRITICO para GitHub Actions
    if resumen["estado"] == "CRITICO":
        log.error("Quality gate FALLIDO — estado CRITICO detectado")
        sys.exit(1)

    print("\n✓ Paso 4 completado. Siguiente: python src/05_visualizacion_drift.py")