"""
src/02_entrenar_modelo.py — Entrena clasificador de calidad de vino y mide la degradación de métricas.
...
if __name__ == "__main__":
    # 1. Verificar existencia de datasets de entrada
    for p in [REF_PATH, PROD_PATH]:
        if not p.exists():
            raise FileNotFoundError(
                f"{p} no encontrado. Ejecuta primero: python src/01_cargar_datos.py"
            )

    # 2. Asegurar existencia de directorio de artefactos
    ARTIFACTS_DIR.mkdir(parents=True, exist_ok=True)

    # 3. Cargar y binarizar datos
    ...
    # 4. Entrenar modelo sólo con datos de Referencia
    ...
    # 6. Evaluar rendimiento y degradación
    ...
    # 9. Guardar datasets con predicciones agregadas
