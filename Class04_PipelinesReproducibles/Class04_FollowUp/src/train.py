"""
src/train.py — Modelado, entrenamiento y análisis de errores.
"""
import logging
...
def entrenar_modelo(nombre, modelo, X_train, y_train, X_test, y_test):
    """Entrena un modelo individual y retorna el objeto fit junto a sus métricas."""
...
def comparar_modelos(X_train, y_train, X_test, y_test, modelos=None):
    """Entrena la suite de modelos y devuelve los resultados ordenados."""
    resultados = {}
    ...
    logger.info('Mejor modelo por F1-Score: %s (F1=%.4f)', mejor, resultados[mejor]['metricas']['f1'])
    return resultados

def analizar_errores(modelo, X_test, y_test):
    """Genera el desglose de la matriz de confusión y reporte de clasificación."""
    y_pred = modelo.predict(X_test)
    ...
    return {
        ...
        'reporte': classification_report(y_test, y_pred, target_names=['No Default', 'Default']),
    }