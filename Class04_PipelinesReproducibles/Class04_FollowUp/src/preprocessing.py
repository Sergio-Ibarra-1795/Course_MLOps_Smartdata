"""
src/preprocessing.py — Limpieza, winsorización, imputación y balanceo.
"""
import logging
import numpy as np
import pandas as pd
...
def limpiar_na_strings(df: pd.DataFrame) -> pd.DataFrame:
    """Convierte valores "NA" como string a NaN."""
    df = df.copy()
...
def winsorizar_columnas(df: pd.DataFrame, columnas=None, p_low=0.05, p_high=0.95) -> pd.DataFrame:
    """Aplica winsorization a percentiles definidos."""
    df = df.copy()
...
def imputar_nulos(df: pd.DataFrame, estrategias=None) -> pd.DataFrame:
    """Imputa valores nulos según estrategia por columna."""
    df = df.copy()
    defaults = {
...
def dividir_y_balancear(X: pd.DataFrame, y: pd.Series, test_size=0.2, tecnica='smote', random_state=42):
    """División estratificada y balanceo de clases (SMOTE, undersampling u oversampling)."""
    ...
    logger.info('Balanceo %s: antes=%d después=%d', tecnica, len(y_train), len(y_bal))
