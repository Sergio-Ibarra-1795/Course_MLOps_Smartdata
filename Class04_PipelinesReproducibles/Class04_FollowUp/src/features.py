"""
src/features.py — Feature engineering y selección de variables.
"""
import logging
import pandas as pd
...
def crear_score_retrasos(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula el score compuesto ponderado de retrasos."""
    df = df.copy()
...
def crear_categorias_edad(df: pd.DataFrame) -> pd.DataFrame:
    """Agrupa Edad en rangos y genera variables dummys."""
    df = df.copy()
...
def crear_categorias_dependientes(df: pd.DataFrame) -> pd.DataFrame:
    """Agrupa Nro_dependiente en rangos y genera variables dummys."""
    df = df.copy()
...
def estandarizar_variables(df: pd.DataFrame, columnas=None) -> pd.DataFrame:
    """Estandariza variables cuantitativas seleccionadas mediante StandardScaler."""
    df = df.copy()
...
def seleccionar_features(X: pd.DataFrame, y: pd.Series, k: int = 15) -> list:
    """Selecciona las mejores K características según el ANOVA F-value (f_classif)."""
    selector = SelectKBest(score_func=f_classif, k='all')
    selector.fit(X, y)
    top_k = pd.Series(selector.scores_, index=X.columns).nlargest(k).index.tolist()
    logger.info('Top %d features seleccionadas: %s', k, top_k[:5])
    return top_k