"""
Secciones 1-7 — Pipeline Base sin Tuning (Línea Base)

Carga (o genera) el dataset en data/raw/, realiza el split 70/30, aplica SMOTE
y entrena un DecisionTree con parámetros por defecto.
Exporta data/processed/base_data.pkl para que los scripts de tuning lo reutilicen.

Ejecución desde la raíz del proyecto:
    python pipelines/01_base_pipeline.py
"""
...
# ── Sección 2: Cargar o generar dataset crudo ────────────────────────────────
DATA_PATH = config.DATASET_PATH

if DATA_PATH.exists():
    df = pd.read_csv(DATA_PATH)
    print(f"✅ Dataset cargado desde '{DATA_PATH}': {df.shape}")
else:
    print(f"⚠️ Dataset no encontrado en '{DATA_PATH}' — generando dataset sintético...")
    np.random.seed(123)
    n = 5000
    df = pd.DataFrame({
    ...
    # Garantizar que la carpeta exista antes de guardar
    DATA_PATH.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(DATA_PATH, index=False)
    print(f"✅ Dataset sintético generado y guardado en '{DATA_PATH}': {df.shape}")
...
# ── Exportar datos procesados para scripts de tuning ──────────────────────────
# Garantizamos que data/processed/ exista
config.PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)

with open(config.BASE_DATA_PKL, "wb") as f:
    pickle.dump((X_train_smote, y_train_smote, X_test, y_test), f)

print(f"\n✓ Datos procesados exportados exitosamente a: {config.BASE_DATA_PKL}")
print("  Siguiente paso: python pipelines/02_gridsearch_mlflow.py")